...
```
~/.agent-lens/browser/                    # All browser data under agent-lens
...
- `agent-lens browser cleanup --force` overrides the exemption
...
agent-lens browser mark "form failed"
# Check storage:
ls ~/.agent-lens/browser/recordings/       # Session directory created
cat ~/.agent-lens/browser/recordings/*/events.jsonl | head  # Events persisted
sqlite3 ~/.agent-lens/browser/index.db "SELECT COUNT(*) FROM events"  # Indexed
ls ~/.agent-lens/browser/recordings/*/screenshots/  # Screenshot captured
