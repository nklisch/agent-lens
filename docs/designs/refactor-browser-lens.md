...
**Approach**:
1. Define `PersistenceConfigSchema` with `dataDir` (string, default `~/.agent-lens/browser`) and `markerPaddingMs` (number, default from buffer config)
...
**Current State**: All browser errors use generic `Error` (10+ locations). Core modules use `AgentLensError` subclasses with error codes.

**Target State**: Browser-specific errors extend `AgentLensError`: `ChromeNotFoundError`, `CDPConnectionError`, `TabNotFoundError`, `BrowserRecorderStateError`.
