...
**Key architectural decision:** The CLI commands do NOT embed `SessionManager` directly. Instead, they communicate with a lightweight background daemon over a Unix domain socket using JSON-RPC. The daemon hosts the same `SessionManager` instance that the MCP server uses. This allows sequential CLI commands (`agent-lens launch`, `agent-lens step`, `agent-lens eval`) to share a persistent debug session.
...
- `getDaemonSocketPath()` checks `process.env.XDG_RUNTIME_DIR` first, then falls back to `path.join(os.homedir(), ".agent-lens")`. Creates the directory if it doesn't exist (`mkdirSync(..., { recursive: true })`). Returns the full path to `agent-lens.sock`.
...
- [ ] `getDaemonSocketPath()` returns `$XDG_RUNTIME_DIR/agent-lens.sock` when env is set
- [ ] `getDaemonSocketPath()` returns `~/.agent-lens/agent-lens.sock` as fallback
...
- **Error mapping:** Catch `AgentLensError` subclasses and map to appropriate `JsonRpcError` codes using the constants from `protocol.ts`. Unknown errors map to `RPC_INTERNAL_ERROR`.
...
- [ ] `AgentLensError` subclasses map to correct RPC error codes
...
**Implementation Notes**:
- **`call()` flow:** Create a `net.Socket()`, connect to `socketPath`. Write the JSON-RPC request as a JSON line + `\n`. Read the response (buffer until `\n`), parse as `JsonRpcResponse`. If `response.error`, throw an `AgentLensError` with the error message and code. Close socket. Use a timeout via `setTimeout` that destroys the socket on expiry.
...
- **`ensureDaemon()` spawn details:**
  ...
  - For compiled binaries, the entry point will be different — the daemon will be a subcommand of the compiled binary itself (`agent-lens _daemon`). Handle both cases: if running from source, spawn `bun run <entry.ts>`; if running as compiled binary, spawn `<binary> _daemon`.
...
- [ ] `call()` throws `AgentLensError` when response contains JSON-RPC error
	...
	async run({ args }) {
		...
		try {
			...
			if (args.exceptions) {
				...
				throw new Error(
					"Usage: agent-lens break <file:line> | --exceptions <filter> | --clear <file>",
				);
			}
		} catch (err) {
			process.stderr.write(formatError(err as Error, mode) + "\n");
...
- **`_daemon` hidden subcommand:** When the compiled binary needs to spawn itself as the daemon, it runs `agent-lens _daemon`. This is a hidden subcommand that imports `src/daemon/entry.ts`.
- **`--break` flag on `launch`:** Currently accepts a single string. For multiple breakpoints, the user calls `agent-lens break` after launching. The `--break` flag on launch is a convenience for the most common case.
...
- `agent-lens skill` prints it to stdout. An agent can load it via `$(agent-lens skill)` or a tool can read it from the known npm path.
- The skill content matches the actual command syntax implemented in Unit 7.

**Acceptance Criteria**:
- [ ] `skill.md` exists at project root
- [ ] `agent-lens skill` prints the skill file to stdout
...
// package.json updates:
{
	...
	"scripts": {
		// ... existing scripts ...
		"build": "bun build --compile src/cli/index.ts --outfile dist/agent-lens",
		"build:linux-x64": "bun build --compile --target=bun-linux-x64 src/cli/index.ts --outfile dist/agent-lens-linux-x64",
		"build:linux-arm64": "bun build --compile --target=bun-linux-arm64 src/cli/index.ts --outfile dist/agent-lens-linux-arm64",
		"build:darwin-x64": "bun build --compile --target=bun-darwin-x64 src/cli/index.ts --outfile dist/agent-lens-darwin-x64",
		"build:darwin-arm64": "bun build --compile --target=bun-darwin-arm64 src/cli/index.ts --outfile dist/agent-lens-darwin-arm64",
		"build:windows-x64": "bun build --compile --target=bun-windows-x64 src/cli/index.ts --outfile dist/agent-lens-windows-x64.exe",
...
- **`agent-lens --version`:** citty handles this via `meta.version`. Should also print platform and Bun version. Add a `setup` hook on the main command to enhance version output.
...
- [ ] `agent-lens --version` prints version, platform, and runtime info
...
bun run build
./dist/agent-lens --version
./dist/agent-lens doctor

# Full CLI debug scenario works
./dist/agent-lens launch "python tests/fixtures/simple.py" --break simple.py:3 --stop-on-entry
./dist/agent-lens step over
./dist/agent-lens eval "x"
./dist/agent-lens stop

# Skill file prints correctly
./dist/agent-lens skill

# JSON mode produces valid JSON
./dist/agent-lens launch "python tests/fixtures/simple.py" --break simple.py:3 --stop-on-entry --json | python3 -m json.tool
```
