...
This guide walks through creating a new language adapter for Agent Lens. By the end, you'll have a working adapter that integrates with the CLI, MCP server, and doctor command.

## Overview

Adapters are the bridge between Agent Lens and language-specific debuggers. Each adapter:
...
export interface PrerequisiteResult {
    ...
    installHint?: string;    // shown in `agent-lens doctor`
}
...
export interface DebugAdapter {
    ...
    displayName: string;           // shown in `agent-lens doctor`
...
`src/adapters/node.ts` and `src/adapters/js-debug-adapter.ts` — downloads the js-debug VSIX to `~/.agent-lens/adapters/js-debug/`, checks cache before downloading, extracts with unzip. TCP transport.
