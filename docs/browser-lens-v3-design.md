...
The investigation tools use the same token-budgeted viewport concept from Agent Lens. Instead of viewing live page state, the agent views recorded session state at a specific point in time. The viewport engine, focus strategies, and token budgeting all carry over — just applied to a timeline of events rather than a live page.
...
Get a structured overview of a session — the "table of contents" before diving in. Token-budgeted, just like Agent Lens viewports.
