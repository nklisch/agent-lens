...
This document defines the formal contracts, detection criteria, event schemas, and bug pattern definitions for Svelte framework state observation in Agent Lens. Svelte is a Tier 3 target.
...
- Useful for debugging but Agent Lens treats this as read-only observation (passive observation principle).
