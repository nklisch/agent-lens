...
This document describes the implementation architecture for the Svelte framework state observer in Agent Lens. It covers the injection strategy, observation flow, component tracking, bug pattern detection, and integration with the broader Browser Lens pipeline.
...
This ensures Agent Lens never degrades the debugging experience — it either adds value or stays out of the way.
