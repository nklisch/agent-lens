import { notNullish, toArray, tryOnScopeDispose, unrefElement } from "./chunk-2CGNBZMC.js";
import { computed, shallowRef, toValue, watch } from "./chunk-NPGVKGNX.js";
...
var matches = NoElement ? () => {} : Element.prototype.matches || Element.prototype.msMatchesSelector || Element.prototype.webkitMatchesSelector;
var getRootNode =
	!NoElement && Element.prototype.getRootNode
		...
		: (element) => (element === null || element === void 0 ? void 0 : element.ownerDocument);
...
var isContentEditable = function isContentEditable2(node) {
	...
	var attValue =
		node === null || node === void 0 ? void 0 : (_node$getAttribute2 = node.getAttribute) === null || _node$getAttribute2 === void 0 ? void 0 : _node$getAttribute2.call(node, "contenteditable");
...
var isDetailsWithSummary = function isDetailsWithSummary2(node) {
	var r = node.tagName === "DETAILS" && Array.prototype.slice.apply(node.children).some((child) => child.tagName === "SUMMARY");
	return r;
};
...
function ownKeys(e, r) {
	...
	if (Object.getOwnPropertySymbols) {
		...
		r && (o = o.filter((r2) => Object.getOwnPropertyDescriptor(e, r2).enumerable)), t.push.apply(t, o);
	}
	return t;
}
...
var createFocusTrap = function createFocusTrap2(elements, userOptions) {
	...
	var updateTabbableNodes = function updateTabbableNodes2() {
		state.containerGroups = state.containers.map((container) => {
			...
			var firstDomTabbableNode = focusableNodes.find((node) => isTabbable(node));
			var lastDomTabbableNode = focusableNodes
				.slice()
				.reverse()
				.find((node) => isTabbable(node));
			var posTabIndexesFound = !!tabbableNodes.find((node) => getTabIndex(node) > 0);
			return {
				container,
				tabbableNodes,
				focusableNodes,
		...
		if (state.containerGroups.find((g) => g.posTabIndexesFound) && state.containerGroups.length > 1) {
	...
	var findNextNavNode = function findNextNavNode2(_ref3) {
		...
		if (state.tabbableGroups.length > 0) {
			...
			if (containerIndex < 0) {
				...
				var startOfGroupIndex = state.tabbableGroups.findIndex((_ref4) => {
					var firstTabbableNode = _ref4.firstTabbableNode;
					return target === firstTabbableNode;
				});
				if (
					startOfGroupIndex < 0 &&
					(containerGroup.container === target || (isFocusable(target, config.tabbableOptions) && !isTabbable(target, config.tabbableOptions) && !containerGroup.nextTabbableNode(target, false)))
				) {
				...
				var lastOfGroupIndex = state.tabbableGroups.findIndex((_ref5) => {
					var lastTabbableNode = _ref5.lastTabbableNode;
					return target === lastTabbableNode;
				});
				if (
					lastOfGroupIndex < 0 &&
					(containerGroup.container === target || (isFocusable(target, config.tabbableOptions) && !isTabbable(target, config.tabbableOptions) && !containerGroup.nextTabbableNode(target)))
				) {
			...
			if (state.mostRecentlyFocusedNode) {
				if (getTabIndex(state.mostRecentlyFocusedNode) > 0) {
					...
					if (tabbableNodes.length > 0) {
						var mruTabIdx = tabbableNodes.findIndex((node) => node === state.mostRecentlyFocusedNode);
						if (mruTabIdx >= 0) {
					...
					if (!state.containerGroups.some((g) => g.tabbableNodes.some((n) => getTabIndex(n) > 0))) {
						navAcrossContainers = false;
					}
				}
			} else {
	...
	var checkDomRemoval = function checkDomRemoval2(mutations) {
		var isFocusedNodeRemoved = mutations.some((mutation) => {
			...
			return removedNodes.some((node) => node === state.mostRecentlyFocusedNode);
		});
	...
	trap = {
		...
		updateContainerElements: function updateContainerElements(containerElements) {
			...
			state.containers = elementsAsArray.map((element) => (typeof element === "string" ? doc.querySelector(element) : element));
			if (config.isolateSubtrees) {
