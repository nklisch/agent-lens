...
var Mark = class {
	...
	checkRanges(array) {
		...
		array
			...
			.forEach((item) => {
				const { start, end, valid } = this.callNoMatchOnInvalidRanges(item, last);
				if (valid) {
					item.start = start;
	...
	wrapRangeFromIndex(ranges, filterCb, eachCb, endCb) {
		this.getTextNodes((dict) => {
			...
			ranges.forEach((range, counter) => {
				const { start, end, valid } = this.checkWhitespaceRanges(range, originalLength, dict.value);
				if (valid) {
					this.wrapRangeInMappedTextNode(
						dict,
						start,
