...
import { addItem, createCart, recalcSubtotal } from "./cart.js";
import { buildCart, checkout } from "./checkout.js";
import { getVolumeTiers, loadConfig } from "./config.js";
import { reserveItems, resetStock } from "./inventory.js";
import { buildLineItems, computeOrderTotal } from "./orders.js";
import { applyVolumePricing, getVolumeTier } from "./pricing.js";
...
describe("Bug 1 — volume tiers sorted lexicographically instead of numerically", () => {
	it("volume tiers must be sorted numerically: [5, 10, 25, 50, 100]", () => {
		...
		for (let i = 1; i < thresholds.length; i++) {
			assert.ok(thresholds[i] > thresholds[i - 1], `Tiers must be in ascending numeric order; got ${thresholds[i - 1]} then ${thresholds[i]}`);
		}
	});
	...
	it("quantity 30 gets bulk-25 tier (10% discount)", () => {
		...
		assert.strictEqual(tier.discount, 0.1, `Expected 10% for qty 30, got ${tier.discount * 100}%`);
	...
	it("quantity 150 gets bulk-100 tier (20% discount)", () => {
		...
		assert.strictEqual(tier.discount, 0.2, `Expected 20% for qty 150, got ${tier.discount * 100}%`);
	});
	...
	it("30 units of WGT-001 ($12.99) with 10% off → line total 350.73", () => {
		...
		assert.strictEqual(item.volumeDiscount, 0.1);
...
describe("Bug 2 — coupon minimum-spend checked against pre-bundle subtotal", () => {
	it("SAVE10 minimum=$50: must NOT apply when post-bundle total is below $50", () => {
		...
		assert.ok(preApplication < 50, `pre-coupon currentTotal should be < 50 (was ${preApplication}) for this test to be meaningful`);
...
describe("Bug 4 — for...in on array iterates Array.prototype.sum key, causing NaN total", () => {
	...
	it("computeOrderTotal returns correct sum: 3*12.99 + 2*9.99 + 5*4.99 = 83.90", () => {
		...
		const total = computeOrderTotal(lineItems);
		assert.ok(Math.abs(total - 83.9) < 0.02, `Expected 83.90, got ${total}`);
	});
