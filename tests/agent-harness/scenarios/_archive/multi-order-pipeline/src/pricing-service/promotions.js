...
export function validateCoupon(code) {
	...
	const expected = createHmac("sha256", COUPON_SECRET).update(`${id}:${rate}`).digest("hex").slice(0, 16);
...
export function applyPromotions(items, orderSubtotal, couponCode) {
	...
	for (const promo of ACTIVE_PROMOTIONS) {
		...
		const eligibleTotal = items.filter((item) => promo.categories.includes(item.category)).reduce((sum, item) => sum + item.lineTotal, 0);
