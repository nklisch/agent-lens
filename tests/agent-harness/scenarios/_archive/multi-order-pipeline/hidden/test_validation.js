...
before(
	async () => {
		...
		await Promise.all([waitForHealth(CATALOG), waitForHealth(PRICING), waitForHealth(ORDERS)]);
	},
	{ timeout: 35_000 },
);
...
describe("Bug 1: electronics shipping (string weight_kg)", () => {
	it("catalog returns numeric weight_kg for ELEC-001", async () => {
		...
		assert.equal(typeof product.weight_kg, "number", `ELEC-001 weight_kg should be a number, got ${typeof product.weight_kg} (${JSON.stringify(product.weight_kg)})`);
		assert.ok(product.weight_kg > 0, `ELEC-001 weight_kg should be > 0, got ${product.weight_kg}`);
	});

	it("catalog returns numeric weight_kg for ELEC-002", async () => {
		const r = await fetch(`${CATALOG}/products/ELEC-002`);
		const product = await r.json();
		assert.equal(typeof product.weight_kg, "number", `ELEC-002 weight_kg should be a number, got ${typeof product.weight_kg}`);
	});
	...
	it("electronics order has non-zero shipping cost", async () => {
		...
		assert.ok(order.shippingCost > 0, `Expected shippingCost > $0 for electronics, got $${order.shippingCost}`);
	});
	...
	it("ELEC-001 shipping matches expected weight (0.15kg × $8 + $3 base = $4.20)", async () => {
		...
		// Total: 16.80
		assert.ok(Math.abs(order.shippingCost - 16.8) < 0.05, `Expected $16.80 shipping, got $${order.shippingCost}`);
	});
});
...
describe("Bug 2: volume discount as fraction vs dollar", () => {
	it("5-unit order applies 7% discount (not $0.07 off)", async () => {
		...
		assert.ok(Math.abs(item.unitPrice - 74.39) < 0.05, `ELEC-004 × 5 should be $74.39 (7% off $79.99), got $${item.unitPrice}`);
	});
	...
	it("10-unit order applies 10% discount correctly", async () => {
		...
		const item = order.items.find((i) => i.productId === "ELEC-004");
		assert.ok(Math.abs(item.unitPrice - 71.99) < 0.05, `ELEC-004 × 10 should be $71.99 (10% off $79.99), got $${item.unitPrice}`);
	});
	...
	it("order total uses pricing service finalPrice, not buggy recomputation", async () => {
		...
		assert.ok(Math.abs(elec.lineTotal - elec.unitPrice * 5) < 0.05, `lineTotal should equal unitPrice × 5`);
	});
});
...
describe("Bug 3: concurrent item pricing order", () => {
	it("ELEC-001 in mixed order has its own price ($29.99)", async () => {
		...
		assert.ok(Math.abs(elec.unitPrice - 29.99) < 0.01, `ELEC-001 should cost $29.99, got $${elec.unitPrice}`);
	});
	...
	it("OFFICE-001 in mixed order has its own price ($12.99)", async () => {
		...
		assert.ok(Math.abs(office.unitPrice - 12.99) < 0.01, `OFFICE-001 should cost $12.99, got $${office.unitPrice}`);
	});
	...
	it("all items in 4-item mixed order have correct prices (10 repeated requests)", async () => {
		...
		for (let trial = 0; trial < 10; trial++) {
			...
			const expected = { "ELEC-001": 29.99, "HOME-001": 45.0, "OFFICE-001": 12.99, "ELEC-002": 39.99 };
			for (const [productId, expectedPrice] of Object.entries(expected)) {
				const item = order.items.find((i) => i.productId === productId);
				assert.ok(item, `${productId} missing from trial ${trial}`);
				assert.ok(Math.abs(item.unitPrice - expectedPrice) < 0.01, `Trial ${trial}: ${productId} should be $${expectedPrice}, got $${item.unitPrice}`);
			}
		}
	});
});
...
describe("Bug 4: catalog pagination preserves category filter", () => {
	it("page 1 of electronics returns only electronics", async () => {
		...
		for (const p of data.products) {
			assert.equal(p.category, "electronics", `Page 1 returned non-electronics: ${p.name} (${p.category})`);
		}
	});
	...
	it("next_page URL includes category parameter", async () => {
		...
		assert.ok(data.next_page.includes("category=electronics"), `next_page "${data.next_page}" should contain "category=electronics"`);
	});
	...
	it("page 2 of electronics returns only electronics", async () => {
		...
		for (const p of page2.products) {
			assert.equal(p.category, "electronics", `Page 2 returned non-electronics: ${p.name} (${p.category})`);
		}
	});
	...
	it("total electronics across all pages matches 12", async () => {
		...
		assert.equal(allElec.length, 12, `Expected 12 electronics products across pages, got ${allElec.length}`);
		for (const p of allElec) {
			assert.equal(p.category, "electronics", `Non-electronics product in paginated result: ${p.name}`);
		}
	});
});
...
describe("Bug 5: price cache respects quantity tiers", () => {
	...
	it("pricing service returns $12.99 base price for 1 unit of OFFICE-001", async () => {
		...
		assert.ok(Math.abs(basePrice - 12.99) < 0.01, `1 unit base price should be $12.99, got $${basePrice}`);
	});
	...
	it("pricing service returns $9.99 base price for 30 units of OFFICE-001 (tier 3)", async () => {
		...
		assert.ok(Math.abs(basePrice - 9.99) < 0.01, `30 units base price should be $9.99 (tier 3: 25+), got $${basePrice} (cached single-unit price)`);
	});
	...
	it("pricing service returns $11.49 base price for 10 units of OFFICE-001 (tier 2)", async () => {
		...
		assert.ok(Math.abs(basePrice - 11.49) < 0.01, `10 units base price should be $11.49 (tier 2: 10-24), got $${basePrice}`);
	});
	...
	it("sequential requests with different quantities return different base prices", async () => {
		...
		assert.ok(base30 < base1, `Tier-3 base price ($${base30}) should be less than tier-1 price ($${base1})`);
	});
});
...
describe("Bug 6: priceSingle Content-Type header", () => {
	it("reprice endpoint returns non-zero price for ELEC-001", async () => {
		...
		assert.ok(repriced.unitPrice > 0, `Reprice should return positive price, got $${repriced.unitPrice}`);
	});
	...
	it("reprice price matches batch pricing for the same item", async () => {
		...
		const repriced = await r.json();
		assert.ok(Math.abs(repriced.unitPrice - batchPrice) < 0.01, `Reprice ($${repriced.unitPrice}) should match batch price ($${batchPrice})`);
	});
	...
	it("reprice for ELEC-001 returns ~$29.99", async () => {
		...
		const repriced = await r.json();
		assert.ok(Math.abs(repriced.unitPrice - 29.99) < 0.01, `ELEC-001 reprice should be ~$29.99, got $${repriced.unitPrice}`);
	});
});
...
describe("Integration: full order pipeline", () => {
	...
	it("order total = subtotal + shipping + tax for a mixed order", async () => {
		...
		const expectedSubtotal = Math.round(order.items.reduce((s, i) => s + i.lineTotal, 0) * 100) / 100;
		assert.ok(Math.abs(order.subtotal - expectedSubtotal) < 0.02, `subtotal mismatch: ${order.subtotal} vs ${expectedSubtotal}`);
		const expectedTotal = Math.round((order.subtotal + order.shippingCost + order.tax) * 100) / 100;
		assert.ok(Math.abs(order.total - expectedTotal) < 0.02, `total mismatch: ${order.total} vs ${expectedTotal}`);
	});
});
