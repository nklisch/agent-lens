...
import { getRoomConfig, loadConfig } from "./config.js";
import { applyLoyalty, finalizeDiscounts } from "./discounts.js";
import { applyGroupDiscount } from "./groups.js";
import { calculateNightlyTotal, calculatePricing, getSeasonalRate } from "./pricing.js";
...
describe("Bug 1 — string resortFee in encoded config causes string concatenation", () => {
	it("deluxe resortFee must be a number, not a string", () => {
		...
		assert.strictEqual(typeof config.roomTypes.deluxe.resortFee, "number", `resortFee should be number, got ${typeof config.roomTypes.deluxe.resortFee} ("${config.roomTypes.deluxe.resortFee}")`);
	});
