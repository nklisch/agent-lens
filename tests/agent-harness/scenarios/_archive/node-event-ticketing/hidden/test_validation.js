...
import { applyDiscount, checkout } from "./checkout.js";
...
import { calculateOrderFees, calculateServiceFee } from "./fees.js";
...
describe("Bug 1 — shallow config merge drops pricing defaults", () => {
	it("loadConfig preserves pricing.baseFee from defaults after override merge", () => {
		...
		assert.strictEqual(typeof config.pricing.baseFee, "number", `config.pricing.baseFee should be a number, got ${typeof config.pricing.baseFee} — Object.assign drops nested objects`);
...
describe("Integration — checkout correct after all bugs fixed", () => {
	...
	it("EVT-002 control checkout succeeds with correct ticket count", async () => {
		clearConfigCache();
		resetLockedSeats();
		const result = await checkout("EVT-002", [{ seatId: "LOWER-F-1" }, { seatId: "LOWER-F-2" }], { payment: TEST_PAYMENT });
