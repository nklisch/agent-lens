...
describe("EventNormalizer WebSocket lifecycle", () => {
	...
	it("normalizes webSocketCreated to websocket event with type='open'", () => {
		const event = normalizer.normalize("Network.webSocketCreated", { requestId: "ws1", url: "wss://example.com/ws" }, "tab1");
	...
	it("normalizes webSocketClosed to websocket event with type='close' and correct URL", () => {
		...
		const event = normalizer.normalize("Network.webSocketClosed", { requestId: "ws2", timestamp: 123456789 }, "tab1");
	...
	it("normalizes loadingFailed for WebSocket to websocket event with type='error'", () => {
		...
		const event = normalizer.normalize("Network.loadingFailed", { requestId: "ws3", errorText: "net::ERR_CONNECTION_REFUSED" }, "tab1");
	...
	it("still normalizes HTTP loadingFailed normally when requestId is not a WebSocket", () => {
		...
		normalizer.normalize("Network.requestWillBeSent", { requestId: "http1", request: { url: "https://example.com/api", method: "GET", headers: {} } }, "tab1");
	...
	it("existing frame normalization (SEND/RECV) is unchanged", () => {
		const sent = normalizer.normalize("Network.webSocketFrameSent", { requestId: "ws4", url: "wss://example.com/ws", response: { payloadData: '{"type":"ping"}' } }, "tab1");

		expect(sent?.type).toBe("websocket");
		expect(sent?.summary).toContain("WS SEND");
		expect(sent?.data.direction).toBe("SEND");

		const recv = normalizer.normalize("Network.webSocketFrameReceived", { requestId: "ws4", url: "wss://example.com/ws", response: { payloadData: '{"type":"pong"}' } }, "tab1");
	...
	it("webSocketClosed with unknown requestId produces event with empty URL", () => {
		const event = normalizer.normalize("Network.webSocketClosed", { requestId: "unknown-ws", timestamp: 123 }, "tab1");
